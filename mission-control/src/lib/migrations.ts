...
const migrations: Migration[] = [
  ...
  {
    id: '043_agent_conversations',
    up(db: Database.Database) {
      // Track persistent conversations between agent pairs
      db.exec(`
      ...
      `)
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_conversations_agents ON agent_conversations(agent_a, agent_b)`)
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_conversations_status ON agent_conversations(status)`)
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_conversations_workspace ON agent_conversations(workspace_id)`)
      db.exec(`CREATE INDEX IF NOT EXISTS idx_agent_conversations_last_msg ON agent_conversations(last_message_at DESC)`)

      // Index for efficient inter-agent message queries
      db.exec(`CREATE INDEX IF NOT EXISTS idx_messages_interagent ON messages(workspace_id, conversation_id) WHERE conversation_id LIKE 'a2a:%'`)
    }
  }
]
